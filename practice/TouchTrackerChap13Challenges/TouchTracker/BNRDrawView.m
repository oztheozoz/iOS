...
- (void)strokeLine:(BNRLine *)line {
    ...
    bp.lineWidth = (line.width) ? line.width : 10;
    
    NSLog(@"bp linewidth = %f", bp.lineWidth);
...
- (void)moveLine:(UIPanGestureRecognizer *)gr {
    ...
    float velocity = hypotf(velocityPoint.x, velocityPoint.y);
    
    //NSLog(@"velocity: ***** hypot = %f", velocity);
    
    if (velocity <= 10) {
        self.selectedLine.width = 10;
    } else if (velocity <= 120) {
        self.selectedLine.width = velocity;
    } else {
        self.selectedLine.width = 120;
    }
    //NSLog(@"selectedLine width = %f", self.selectedLine.width);
