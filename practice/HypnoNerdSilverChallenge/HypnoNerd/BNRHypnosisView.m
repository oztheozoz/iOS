...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        self.segCtrl = [[UISegmentedControl alloc] initWithItems:@[@"Red", @"Green", @"Blue"]];
        CGRect segCtrlBounds = CGRectMake(0, 30, self.bounds.size.width, 50);
        self.segCtrl.frame = segCtrlBounds;
        [self.segCtrl setSelectedSegmentIndex:0];
        [self.segCtrl addTarget:self action:@selector(whichColor:) forControlEvents:UIControlEventValueChanged];
...
- (void)whichColor:(UISegmentedControl *)paramSender {
    if([paramSender isEqual:self.segCtrl]) {
